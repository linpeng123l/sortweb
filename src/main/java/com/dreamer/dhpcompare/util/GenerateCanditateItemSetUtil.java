        ...
        for (int i = 0; i < preFrequentItemSets.size(); i++) {
            ...
            for (int j = i + 1; j < preFrequentItemSets.size(); j++) {
                ...
                if (match && !firstBreaks[index].equals(secondBreaks[index])) {
                    ...
                    if(Integer.parseInt(secondBreaks[level - 1])>Integer.parseInt(firstBreaks[level-1])){
                        items[level-1] = firstBreaks[level - 1];
                        items[level] = secondBreaks[level - 1];
                    }else {
                        items[level-1] = secondBreaks[level - 1];
                        items[level] = firstBreaks[level - 1];
                    }
